      ...
      <div className="flex  flex-col gap-4  justify-center relative w-full h-full bg-white rounded-lg p-10 sm:p-8  mb-12">
        <div className="  flex flex-col items-center justify-center bg-white gap-4 ">
          <div className=" h-[50px] w-[50px]">
            {selectedCard.renderIcon(selectedCard.color + " " + "text-5xl ")}
          </div>
          <h1
            className={`text-3xl sm:text-2xl whitespace-nowrap mx-auto  ${selectedCard.color} font-medium`}
          >
            {selectedCard.title}
          </h1>
        </div>
        ...
        {cards.map((card, index) => {
          return (
            <div
              ...
              key={index}
              className={`cursor-pointer w-[75px] h-[75px] shadow-md  ${
                selectedCard.id !== card.id
                  ? "bg-white "
                  : card.backgroundColor + " " + "scale-110"
              } transition-all border border-1 border-purple-300 rounded-xl flex items-center justify-center `}
            >
              {card.renderIcon(
                selectedCard.id !== card.id ? card.color : "text-white"
              )}
            </div>
          );
        })}
